...
void TextEdit::clampPos(TextPos& p) const
{
    ...
    if (p.line >= lc) p.line = lc - 1;
    int cc = lineColCount(p.line);
    if (p.col < 0) p.col = 0;
    if (p.col > cc) p.col = cc;
}
...
void TextEdit::paint(PaintContext& ctx)
{
    ...
    if (firstVRow < 0) firstVRow = 0;
    if (lastVRow >= totalVRows) lastVRow = totalVRows - 1;
    ...
    auto alignOffset = [&](const std::string& rowText, float areaW) -> float {
        if (align_ == TextAlign::LEFT) return 0;
        float tw = ctx.font.GetTextWidth(rowText.c_str());
        if (align_ == TextAlign::CENTER) return std::max(0.0f, (areaW - tw) * 0.5f);
        if (align_ == TextAlign::RIGHT)  return std::max(0.0f, areaW - tw);
    ...
    for (int vr = firstVRow; vr <= lastVRow; ++vr)
    {
        ...
        if (isFocused() && hasSelection())
        {
            ...
            if (rowEnd >= s && rowStart <= e)
            {
                ...
                else if (i == s.line && i == e.line) { selS = std::max(startCol, s.col); selE = std::min(endCol, e.col); }
                else if (i == s.line) { selS = std::max(startCol, s.col); selE = endCol; }
                else if (i == e.line) { selS = startCol; selE = std::min(endCol, e.col); }
        ...
        if (align_ == TextAlign::JUSTIFY && rowIdx < static_cast<int>(rows.size()) - 1 && !rowText.empty())
        {
            ...
            {
                float totalWordW = 0;
                for (auto& w : words) totalWordW += ctx.font.GetTextWidth(w.c_str());
...
void TextEdit::onKeyPress(KeyEvent& e)
{
    ...
    case SDLK_UP:
        if (cursor_.line > 0)
        {
            ...
            if (cursor_.col > cc) cursor_.col = cc;
        }
        if (!e.shift) selAnchor_ = cursor_;
        ensureCursorVisible();
        markDirty();
    ...
    case SDLK_TAB:
    {
        if (readOnly_) { e.consumed = true; break; }
