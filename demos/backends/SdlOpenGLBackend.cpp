...
bool SdlOpenGLBackend::createDeviceObjects()
{
    const char* vs = R"(
        ...
        // Column-major 3×3 2D homogeneous transform (scale+rotate+translate).
        // Applied before the ortho projection.
...
// ─────────────────────────────────────────────────────────────────────────────
// Build a column-major 3×3 camera matrix from Camera2D.
// Transform: T(pivot) * R*S * T(-pivot + cam.xy)
//   where pivot = (cam.pivotX * W, cam.pivotY * H)
// The resulting mat is: col0=[c,s,0], col1=[-s,c,0], col2=[tx,ty,1]
// ─────────────────────────────────────────────────────────────────────────────
                                          ...
                                          float displayW, float displayH)
{
    ...
    // offset = (cam.x - pivX, cam.y - pivY)  mapped through R*S, then + pivot
    float ox = cam.x - pivX;
    float oy = cam.y - pivY;
...
void SdlOpenGLBackend::renderDrawData(const BuGUI::DrawData& drawData)
{
    ...
    for (const BuGUI::DrawPass& pass : drawData.passes)
    {
        ...
        // Pre-compute clip transform coefficients (angle==0 fast path)
        ...
        for (const BuGUI::DrawCmd& cmd : commands)
        {
            ...
            if (noRotation)
            {
                // Transform clip rect by the same affine (scale+translate)
                cx0 = c * cmd.clip.x               + tx;
                cy0 = c * cmd.clip.y               + ty;
            ...
            {
                // Rotated: conservative full-viewport scissor
                cx0 = 0.0f; cy0 = 0.0f;
            ...
            GLuint texId = cmd.texture ? static_cast<GLuint>(cmd.texture.value) : 0;
            if (cmd.texture)
            {
            ...
            if (texId != lastTexture) { stats.textureSwitches++; lastTexture = texId; }
...
void SdlOpenGLBackend::applyCursor(int cursorType)
{
    ...
    static const SDL_SystemCursor sdlMap[] = {
        ...
        SDL_SYSTEM_CURSOR_HAND,      // 1 Hand
        SDL_SYSTEM_CURSOR_IBEAM,     // 2 IBeam
        SDL_SYSTEM_CURSOR_CROSSHAIR, // 3 Crosshair
        SDL_SYSTEM_CURSOR_SIZEWE,    // 4 SizeH
        SDL_SYSTEM_CURSOR_SIZENS,    // 5 SizeV
        SDL_SYSTEM_CURSOR_SIZEALL,   // 6 SizeAll
        SDL_SYSTEM_CURSOR_NO,        // 7 No
    };
