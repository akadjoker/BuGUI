...
void Font::drawTextCodepoint(int codepoint, float x, float y)
{
    ...
    // Clipping — soft clip: trim glyph rect and adjust UVs proportionally
    if (enableClip)
    {
        float cx0 = clip.x;
        float cy0 = clip.y;
        ...
        float dx0 = dstRec.x;
        float dy0 = dstRec.y;
        float dx1 = dstRec.x + dstRec.width;
        float dy1 = dstRec.y + dstRec.height;

        // Fully outside — skip
        if (dx1 <= cx0 || dx0 >= cx1 || dy1 <= cy0 || dy0 >= cy1)
            return;

        // Compute how much to trim (0..1 ratio)
        float trimL = (dx0 < cx0) ? (cx0 - dx0) / dstRec.width  : 0.0f;
        float trimR = (dx1 > cx1) ? (dx1 - cx1) / dstRec.width  : 0.0f;
        float trimT = (dy0 < cy0) ? (cy0 - dy0) / dstRec.height : 0.0f;
        float trimB = (dy1 > cy1) ? (dy1 - cy1) / dstRec.height : 0.0f;

        // Adjust dst rect
        dstRec.x     += trimL * dstRec.width;
        dstRec.width -= (trimL + trimR) * dstRec.width;
        dstRec.y      += trimT * dstRec.height;
        dstRec.height -= (trimT + trimB) * dstRec.height;

        // Adjust src rect (UV) proportionally
        float srcW = srcRec.width;
        float srcH = srcRec.height;
        srcRec.x     += trimL * srcW;
        srcRec.width -= (trimL + trimR) * srcW;
        srcRec.y      += trimT * srcH;
        srcRec.height -= (trimT + trimB) * srcH;
...
void Font::Print(const char *text, float x, float y, const ClipRect* clipRect)
{
    if (!clipRect)
    {
        Print(text, x, y);
    ...
    bool prevEnable = enableClip;
    IntRect prevClip = clip;
    enableClip = true;
    clip = IntRect(static_cast<int>(clipRect->x), static_cast<int>(clipRect->y),
                   static_cast<int>(clipRect->w), static_cast<int>(clipRect->h));
    Print(text, x, y);
    enableClip = prevEnable;
    clip = prevClip;
}
...
bool Font::LoadDefaultFont()
{
    ...
    // Use embedded Noto Sans TTF for high-quality default font
    #include "NotoSans_embedded.h"
    return LoadTTFFromMemory(NotoSans_ttf_data, NotoSans_ttf_size, 16.0f, 256);
#else
    ...
    texture = CreateTextureFromPixmap("FontDefault", pixmap);
    ...
    SDL_Log("[Font] Default bitmap font loaded (%d glyphs)", m_glyphCount);
...
bool Font::LoadBMFont(const char* fntPath, const char* texturePath)
{
   ...
   // int scaleW = 0, scaleH = 0;
    ...
    texture = CreateTextureFromPixmap(textureFile.c_str(), pixmap);
                         ...
                         const char* charset)
{
    ...
    // Create texture
    texture = CreateTextureFromPixmap("FontCustom", pixmap);
...
bool Font::LoadTTF(const char* ttfPath, float size, int atlasSize)
{
    ...
    if (fileSize <= 0)
    {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "[Font] Failed to get TTF file size: %s", ttfPath);
        SDL_RWclose(rw);
        return false;
    }
    
    std::vector<unsigned char> ttfBuffer((size_t)fileSize);
    if (SDL_RWread(rw, ttfBuffer.data(), 1, (size_t)fileSize) != (size_t)fileSize)
    {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "[Font] Failed to read TTF file: %s", ttfPath);
        SDL_RWclose(rw);
        return false;
    }
    SDL_RWclose(rw);
                            ...
                            float size, int atlasSize)
{
    ...
    // Build list of codepoints to load:
    //   ASCII 32-126, Latin-1 Supplement 160-255, Latin Extended-A 256-383,
    //   General Punctuation, Currency, Arrows, Math, Box Drawing, Geometric Shapes
    std::vector<int> codepoints;
    // ASCII printable
    for (int c = 32; c <= 126; c++) codepoints.push_back(c);
    // Latin-1 Supplement (¡ through ÿ — includes ¥ ©  ® ± × ÷ accented chars)
    for (int c = 160; c <= 255; c++) codepoints.push_back(c);
    // Latin Extended-A (Ā through ſ — Eastern European)
    for (int c = 256; c <= 383; c++) codepoints.push_back(c);
    // General Punctuation (–—''""•…)
    for (int c = 0x2010; c <= 0x2027; c++) codepoints.push_back(c);
    // Currency Symbols (₠ through ₿ — includes €)
    for (int c = 0x20A0; c <= 0x20BF; c++) codepoints.push_back(c);
    // Arrows (← ↑ → ↓ etc.)
    for (int c = 0x2190; c <= 0x21FF; c++) codepoints.push_back(c);
    // Math Operators (∀ ∂ ∃ … ≠ ≡ ≤ ≥ etc.)
    for (int c = 0x2200; c <= 0x22FF; c++) codepoints.push_back(c);
    // Box Drawing (─ │ ┌ etc.)
    for (int c = 0x2500; c <= 0x257F; c++) codepoints.push_back(c);
    // Block Elements (▀ ▄ █ etc.)
    for (int c = 0x2580; c <= 0x259F; c++) codepoints.push_back(c);
    // Geometric Shapes (■ □ ▲ ● ○ etc.)
    for (int c = 0x25A0; c <= 0x25FF; c++) codepoints.push_back(c);
    // Miscellaneous Symbols subset (☐ ☑ ☒ ★ ☆ etc.)
    for (int c = 0x2600; c <= 0x26FF; c++) codepoints.push_back(c);
    
    // Filter out codepoints the font doesn't have
    std::vector<int> validCodepoints;
    validCodepoints.reserve(codepoints.size());
    for (int cp : codepoints)
    {
        if (stbtt_FindGlyphIndex(&font, cp) != 0)
            validCodepoints.push_back(cp);
    }
    
    // Create atlas (2 channels: grayscale + alpha)
    ...
    m_glyphCount = (int)validCodepoints.size();
    m_glyphs.resize(m_glyphCount);
    m_recs.resize(m_glyphCount);
    m_glyphPadding = 1;
    ...
    for (int i = 0; i < m_glyphCount; i++)
    {
        int codepoint = validCodepoints[i];
        ...
        if (y + height + 1 > atlasSize)
        {
            SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                "[Font] Atlas full at glyph %d/%d (U+%04X). Increase atlasSize.",
                i, m_glyphCount, codepoint);
            if (bitmap) stbtt_FreeBitmap(bitmap, nullptr);
            // Truncate to what we packed so far
            m_glyphCount = loaded;
            m_glyphs.resize(m_glyphCount);
            m_recs.resize(m_glyphCount);
            break;
        ...
        if (bitmap)
        {
            for (int by = 0; by < height; by++)
            {
                for (int bx = 0; bx < width; bx++)
                {
                    int atlasX = x + bx;
                    int atlasY = y + by;
                    u8 v = bitmap[by * width + bx];
                    atlas.SetPixel(atlasX, atlasY, 255, 255, 255, v);
                }
            }
    ...
    texture = CreateTextureFromPixmap("FontTTF", atlas);
    if (!texture)
    {
    ...
    SDL_Log("[Font] TTF font loaded (%.0fpx, %d glyphs, atlas %dx%d)",
            size, m_glyphCount, atlasSize, atlasSize);
